...
namespace AntDesign
{
    public class ColumnBase : AntDomComponentBase, IColumn
    {
        ...
        [CascadingParameter] public ColumnContext Context { get; set; }
        ...
        [Parameter] public string Fixed { get; set; }

        [Parameter] public RenderFragment ChildContent { get; set; }

        public int ColIndex { get; set; }

        protected string FixedStyle => Fixed != null
            ? $"position: sticky; {Fixed}: {Width * (Fixed == "left" ? ColIndex : Context.Columns.Count - ColIndex - 1)}px;"
            : "";
        ...
        private void SetClass()
        {
            ClassMapper
                ...
                .If($"ant-table-cell-fix-right-first",
                    () => Fixed == "right" && Context?.Columns.FirstOrDefault(x => x.Fixed == "right")?.ColIndex ==
                        this.ColIndex)
                .If($"ant-table-cell-fix-left-last",
                    () => Fixed == "left" && Context?.Columns.LastOrDefault(x => x.Fixed == "left")?.ColIndex ==
                        this.ColIndex)
                ;
        }
