...
namespace AntDesign
{
    ...
    public partial class Descriptions : AntDomComponentBase
    {
        ...
        [Parameter] public string Layout { get; set; } = DescriptionsLayout.Horizontal;

        [Parameter] public ColumnType Column { get; set; }

        [Parameter] public string Size { get; set; }

        [Parameter] public OneOf<string, RenderFragment> Title { get; set; }

        [Parameter] public bool Colon { get; set; }
        ...
        private List<List<(IDescriptionsItem item, int realSpan)>> _itemMatrix =
            new List<List<(IDescriptionsItem item, int realSpan)>>();
        ...
        private static readonly List<(int PixelWidth, BreakpointEnum Breakpoint)> _descriptionsResponsiveMap =
            new List<(int, BreakpointEnum)>()
            {
        ...
        private void PrepareMatrix()
        {
            ...
            List<(IDescriptionsItem item, int realSpan)>
        ...
        private async Task SetRealColumn()
        {
            ...
            else
            {
                ...
                var breakpointTuple =
                    _descriptionsResponsiveMap.FirstOrDefault(x => x.PixelWidth > element.clientWidth);
                var bp = breakpointTuple == default ? BreakpointEnum.xxl : breakpointTuple.Breakpoint;
                _realColumn = Column.AsT1.ContainsKey(bp.ToString())
                    ? Column.AsT1[bp.ToString()]
                    : _defaultColumnMap[bp.ToString()];
            }
        }
    }
}
