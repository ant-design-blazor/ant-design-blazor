...
| OnBlur | Called when range item looses focus. | EventCallback<(double, double)> | - | 0.11
...
| OnFocus | Called when range item recives focus. | EventCallback<(double, double)> | - | 0.11
...
#### MultiRangeSlider Common Methods

| Property | Description | Parameters | Version |
| --- | --- | --- | --- |
| Focus() | If multiple `RangeItem`s have the same value (overlapping), then fisrt matching will receive focus. | (double, double) value | 0.11
| Blur() | Removes focus. | - | 0.11
...
| Focused | When `true` the range item will receive focus. When multiple `RangeItem`s have this parameter set, then the last one prevails.  | bool | false | 0.11
