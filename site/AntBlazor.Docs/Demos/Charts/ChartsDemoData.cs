...
namespace AntDesign.Charts
{
    public static class ChartsDemoData
    {
        public static async Task<FireworksSalesItem[]> FireworksSalesAsync(NavigationManager navigationManager, HttpClient httpClient)
        {
            ...
            return await httpClient.GetFromJsonAsync<FireworksSalesItem[]>(new Uri(baseUrl, "_content/AntDesign.Docs/chartsdata/fireworks-sales.json").ToString());
        }

        public static async Task<SalesItem[]> SalesAsync(NavigationManager navigationManager, HttpClient httpClient)
        {
            var baseUrl = navigationManager.ToAbsoluteUri(navigationManager.BaseUri);
            return await httpClient.GetFromJsonAsync<SalesItem[]>(new Uri(baseUrl, "_content/AntDesign.Docs/chartsdata/sales.json").ToString());
        }

        public static async Task<GDPItem[]> GDPAsync(NavigationManager navigationManager, HttpClient httpClient)
        {
            var baseUrl = navigationManager.ToAbsoluteUri(navigationManager.BaseUri);
            return await httpClient.GetFromJsonAsync<GDPItem[]>(new Uri(baseUrl, "_content/AntDesign.Docs/chartsdata/GDP.json").ToString());
        }

        public static async Task<EmissionsItem[]> EmissionsAsync(NavigationManager navigationManager, HttpClient httpClient)
        {
            var baseUrl = navigationManager.ToAbsoluteUri(navigationManager.BaseUri);
            return await httpClient.GetFromJsonAsync<EmissionsItem[]>(new Uri(baseUrl, "_content/AntDesign.Docs/chartsdata/emissions.json").ToString());
        }

        public static async Task<OilItem[]> OilAsync(NavigationManager navigationManager, HttpClient httpClient)
        {
            var baseUrl = navigationManager.ToAbsoluteUri(navigationManager.BaseUri);
            return await httpClient.GetFromJsonAsync<OilItem[]>(new Uri(baseUrl, "_content/AntDesign.Docs/chartsdata/oil.json").ToString());
        }

        public static async Task<IMDBItem[]> IMDBAsync(NavigationManager navigationManager, HttpClient httpClient)
        {
            var baseUrl = navigationManager.ToAbsoluteUri(navigationManager.BaseUri);
            return await httpClient.GetFromJsonAsync<IMDBItem[]>(new Uri(baseUrl, "_content/AntDesign.Docs/chartsdata/IMDB.json").ToString());
        }

        public static async Task<SmokingRateItem[]> SmokingRateAsync(NavigationManager navigationManager, HttpClient httpClient)
        {
            var baseUrl = navigationManager.ToAbsoluteUri(navigationManager.BaseUri);
            return await httpClient.GetFromJsonAsync<SmokingRateItem[]>(new Uri(baseUrl, "_content/AntDesign.Docs/chartsdata/smoking-rate.json").ToString());
        }
        ...
        {
            var baseUrl = navigationManager.ToAbsoluteUri(navigationManager.BaseUri);
            return await httpClient.GetFromJsonAsync<JobpayingItem[]>(new Uri(baseUrl, "_content/AntDesign.Docs/chartsdata/jobpaying.json").ToString());
        }
        ...
        {
            var baseUrl = navigationManager.ToAbsoluteUri(navigationManager.BaseUri);
            return await httpClient.GetFromJsonAsync<ContributionsItem[]>(new Uri(baseUrl, "_content/AntDesign.Docs/chartsdata/contributions.json").ToString());
        }
